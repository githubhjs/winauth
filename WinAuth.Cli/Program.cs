  ...
  internal static class Program {
    static int Main(string[] args) {
      try {
        ...
        else if (command == "show") { var e=Find(vault,Need(a,0,"name or id")); Console.WriteLine("otpauth://{0}/{1}?secret={2}&issuer={3}&digits={4}&period={5}&counter={6}&algorithm={7}",e.Kind,Uri.EscapeDataString(e.Name),e.Secret,Uri.EscapeDataString(e.Issuer??""),e.Digits,e.Period,e.Counter,e.Algorithm); }
...
Commands:
  list
  code NAME|ID
  add --name NAME --secret BASE32 [--type totp|hotp] [--issuer TEXT]
